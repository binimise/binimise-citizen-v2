...
import { getSpotFine, updateUserToken, updateSaathiNotMyWork,getAppSettings, updateSaathiWorkDoneImage } from "../repo/repo";
...
export default ({ navigation }) => {
    ...
    const [spotfine, setSpotfine] = useState([]);
    ...
    const getDynamicDataFromSettings = async (Language)=>{
    ...
    const getAllSpotfine = async () => {
        let state = await NetInfo.fetch();
        if (!state.isConnected) {
            let message =  "you_are_offline"
            return setDataAction({ 
                    errorModalInfo : { 
                        showModal : true, message, onClose : getAllSpotfine
                    }
                });
        }
        let spot_fine = await getSpotFine(userInfo);
        if(!spot_fine.length){
            spot_fine = [];
        }
        setSpotfine(spot_fine);
        setIsFetching(false);
        setIsLoading(false);
    ...
    const getSaathiCurrentLocationInSpotfine = async()=>{
        let saathi_loc = {};
        try{
    ...
    const updateSpotfineStatus = async (state) => {
        if (!userInfo.status) {
            ...
            <View w={width - 48} br={8} c={Color.white} jc pa={16} mt={10}>
                ...
                </View>
                {
                    complaint?.photo_url ? <View mt={10}>
                        <Image uri={complaint?.imageUrl} resizeMode="cover" h={120} /> 
                    </View> : null
                }
    ...
    const _showNotifModalC = async(item) => {
        ...
        await getSaathiCurrentLocationInSpotfine();
    }

    const _showNotMyWorkModalC = () => {
        return (
            <Modal>
                <View pa={8}>
                ...
                <View row c={Color.white} mv={8}>
                    ...
                    <Touch jc mt={20} mb={5} h={36} w={'48%'} br={4} bw={1}
                        ...
                        onPress={e => {
                            setShowCommentModalC(false);
                            updateSpotfineStatus(WORK_STATUS_NOT_MY_WORK);
                        }} />
                </View>
            </Modal>
        );
    ...
    const updateNotMyWork = async () => {
        let obj = { imageUrl, comment, date: new Date().getTime() };
        updateSaathiNotMyWork(complaint.id, obj, complaint.type);
        getAllSpotfine();
    }

    const cameraOnload = async (url) => {
        setImageUrl(url);
    }
    const cameraOnloadP = async (url) =>{
        setImagePUrl(url)
    }

    const _showDoneModalC = () => {
        return (
            <Modal>
                <View pa={8}>
                    <Text t={"please_enter_your_comment"} />
                ...
                <View row c={Color.white} mv={8}>
                    ...
                    <Touch jc mb={5} h={36} w={'48%'} br={4} bw={1}
                        ...
                        onPress={e => {
                            setshowDoneModalCC(false);
                            updateSpotfineStatus(WORK_STATUS_DONE);
                        }} />
                </View>
            </Modal>
        );
    }
    const _showPriorModalC = () =>{
        return (
            <Modal>
                <View pa={8}>
                    <Text t={"please_enter_your_comment"} />
    ...
    return (
        <View>
            <Header navigation={navigation} headerText={"spot_fine"} />
            {userInfo.status?
            <View>
                <View ph={16} mv={8}>
                ...
                }
                {
                        ...
                        renderItem={({ item, index }) => (
                            <View bw={1} bc={Color.lightGrayColor} mb={20} br={4} ph={16}>
                                        <View row ai style={{justifyContent: "space-between"}} h={36}>
                                            <Text s={14} c={Color.black} t={new Date(item.timeStamp?.seconds * 1000).toDateString()} />
                                            <Text s={16} b c={Color.themeColor} t={item.state || "ACTIVE"} />
                                        </View>
                                        <View row ai style={{justifyContent: "space-between"}} h={36}>
                                            <Text s={14} c={Color.black} t={item.name || "N/A"} />
                    ...
                    <View mb={180} />
                    { showNotifModalC ? spotfineModal() : null }
