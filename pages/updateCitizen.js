...
import { getUserByQrCode, getAllAreas,checkUsersData, getCitizenData, updateCitizenData,getUserData } from "./../repo/repo";
...
export default ({ navigation }) => {
    ...
    formOnChangeEditUser = (field, value) => {
    ...
    updateCitizenInfo = async () => {
        ...
        obj["userType"] = "user"
        formOnChangeEditUser(USERINFO, obj);
    }
    ...
    getImageDetails= async(region)=>{
        ...
        await Location.reverseGeocodeAsync({latitude:region.latitude,longitude: region.longitude}).
          then(result => {
            ...
            formOnChangeEditUser("address",_address)
          });
      };
    ...
    getUpdateCitizenView = (text, ph, name, value, keyboardType, maxLength) => {
        return (
            
            <View style={styles.cardStyle}>
                <Text s={12} t={text} c={"black"} b/>
                <TextInput ml nl={1} ph={ph} pl={"4%"} h={40} bc={"white"} bbw= {1}
                    onChangeText={formOnChangeEditUser} name={name} tbc={"#F0F0F0"} mb={4}
    ...
    searchUser = async () => {
        ...
        toggleLoading(true);
        let _citizenInfo = await getCitizenData(state.phoneNumber);
        if(!_citizenInfo) {
            toggleLoading(false);
            return showErrorModalMsg("no_user_with_this_number_exists");
        }
        let loc_obj={latitude:_citizenInfo.lat?_citizenInfo.lat:APP_CONFIG.COORDINATES.coords.latitude,
            longitude:_citizenInfo.long?_citizenInfo.long:APP_CONFIG.COORDINATES.coords.longitude}
    
        setRegion({ ...loc_obj, latitudeDelta: 0.01, longitudeDelta: 0.01 });
        setScannedValue(_citizenInfo?.qrCode?_citizenInfo.qrCode:"")
      
        formOnChangeEditUser(USERINFO,_citizenInfo);
    ...
    getScannedValue = scannedValue => {
        if(!scannedValue){
         ...
         isSearching? searchUserByQr(scannedValue):setScannedValue(scannedValue)
       
     }
 

    searchUserByQr = async (scannedValue) => {
        
        toggleLoading(true);
        let _citizenInfo = await getUserByQrCode(scannedValue);
        if(!_citizenInfo) {
            toggleLoading(false);
        ...
        setRegion({ ...loc_obj, latitudeDelta: 0.01, longitudeDelta: 0.01 });
        //   console.log("c",_citizenInfo)
        setScannedValue(_citizenInfo?.qrCode)
                ...
                <View row mb={4}>
                    <View br={4} bc={Color.lightGrayColor}c={"white"} w={width - 80} bw={1} pt={4} pb={4} mb={8}>
                        ...
                        <TextInput ml nl={1} ph={'9989443788'} pl={16} h={24}
                            ...
                            onChangeText={formOnChangeEditUser} name={"phoneNumber"}
    ...
    selectedPickerData = (key,data)=>{
      ...
      formOnChangeEditUser(key,data)
    }
    // console.log("s in citizen",state)
    return (
        <View>
            {showScanner ? 
                ...
                <View c={"#F0F0F0"} w={"100%"} h={"100%"}>
                    ...
                    <Header navigation={navigation} headerText={"updateResident"}/>
                    <ScrollView contentContainerStyle={{ width:"90%",marginHorizontal:"5%"}}>
                        {
                            searchByName ? searchByNameView() : searchByPhone()
                        }
                        <Text t={"update_details"} mb={4}/>
                        {
                            getUpdateCitizenView('name', 'firstName_lastName', 'name', state.name)
                        }

                        {
                            getUpdateCitizenView('phoneNumber', '9954672326', 'phoneNumber', state.phoneNumber, "numeric", 10)
                        }
                        ...
                        <View w={"100%"} mb={"4%"} bc={Color.black} bw={1}  br={4} c={"white"}>
                            {
                                getUpdateCitizenView('address', 'address', 'address', state.address)
                            }
